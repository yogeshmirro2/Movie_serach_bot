...
from bot.plugins.verify_helper import get_verify_date_return_string,get_updated_date
class Database:
    ...
    async def new_user(self, id):
        return dict(
            ...
            valid_date= await get_verify_date_return_string(Config.VERIFY_DAYS) if Config.VERIFY_DAYS else await get_verify_date_return_string("10"),
            verify_key = secrets.choice(Config.VERIFY_KEY) if Config.VERIFY_KEY else "".join(secrets.choice(string.ascii_uppercase+string.digits+string.ascii_lowercase) for i in range(7))
        )
    ...
    async def get_verify_key(self,id):
        status = await self.ucol.find_one({'id' : int(id)})
        ...
        if Config.VERIFY_KEY:
            if user_key in Config.VERIFY_KEY:
                ...
                await self.ucol.update_one({'id': id}, {'$set' : {'verify_key': verify_key}})
                user_key = status.get('verify_key')
        return user_key
    ...
    async def update_verify_key(self,id):
        if Config.VERIFY_KEY:
            ...
            await self.ucol.update_one({'id': id}, {'$set' : {'verify_key': updates}})
        else:
            updates = "".join(secrets.choice(string.ascii_uppercase+string.digits+string.ascii_lowercase) for i in range(7))
            await self.ucol.update_one({'id': id}, {'$set' : {'verify_key': updates}})
    ...
    async def get_verify_date(self,id):
        user = await self.ucol.find_one({'id': int(id)})
        user_date = user.get('valid_date')
        return user_date
        
    async def update_verify_date(self , id):
        updates = await get_updated_date(Config.VERIFY_DAYS)
        await self.ucol.update_one({'id': id}, {'$set': {'valid_date': updates}})
    ...
    async def get_filters(self, group_id: int, keyword: str):
        ...
        chat_accuracy = float(chat["configs"].get("accuracy"))
        achats = await self.find_active(group_id)
        ...
        filters = []
        keyword2 = Config.ADDITIONAL_KEYWORD+keyword
        pipeline= {
            'group_id': int(group_id), '$text':{'$search': f"{keyword} {keyword2}"}
        }
        ...
        for document in await db_list.to_list(length=600):
            print(f"{document['score']}:{chat_accuracy}")
